...
export const USAGE = `/quick-review [--base <ref>] [--target <ref>] [--repo <path>] [--no-open]
  ...
  With no --base, analyze one committed target snapshot (HEAD by default).
  With --base, analyze a base-to-target diff (target defaults to HEAD).

  --base <ref>     Base of a diff graph.
  --target <ref>   Target commit. Defaults to HEAD.
  --repo <path>    Repository to analyze. Defaults to the session directory.
